import {ApolloError, ApolloQueryResult, DocumentNode, OperationVariables, QueryResult, TypedDocumentNode, useApolloClient, useQuery,} from '@apollo/client';
...
const createQueryResultMock = <TData, TVariables extends OperationVariables>(
  ...
  return {
    ...
    refetch: (() => Promise.resolve(
                  {data: undefined} as ApolloQueryResult<TData>)) as any,
...
>(
  ...
  const getCachedResult = useCallback((): QueryCacheEntry<TData>|undefined => {
      ...
      async():
          Promise<QueryResult<TData, TVariables>> => {
            ...
            if (inFlightQueries.has(cacheKey)) {
              return inFlightQueries.get(cacheKey) as
                  Promise<QueryResult<TData, TVariables>>;
            }
            ...
            const queryPromise =
                client
                    ...
                    .then((result) => {
                      ...
                      const dataToCache =
                          updateCache ? updateCache(result.data) : result.data;
                      ...
                      return createQueryResultMock<TData, TVariables>({
                        ...
                        refetch: (() => executeQuery()) as
                            EnhancedRefetchFunction<TData, TVariables>,
                    ...
                    .catch(async (error) => {
                      ...
                      if (logErrors) {
                        console.error(
                            `Error executing query ${queryName}:`, error);
                      }
  ...
  useEffect(
      () => {
        ...
        if (skip) {
          setResult(
              (prev) =>
                  ({...prev, loading: false, called: true, data: undefined}));
          return;
        }
