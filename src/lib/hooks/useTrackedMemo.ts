...
export function useTrackedMemo<T>(
  ...
  // Use normal useMemo for the actual memoization with explicitly included
  // dependencies for all values that are used inside the function
  return useMemo(() => {
    ...
    if (verbose || computationTime > warnThreshold) {
      ...
      if (computationTime > warnThreshold) {
        console.warn(
          `Expensive computation detected: "${name}" took ${computationTime.toFixed(
            2,
          )}ms. ` +
  ...
  }, [factory, enabled, name, verbose, warnThreshold, ...deps]);
}
...
export function useTrackedCallback<T extends (...args: any[]) => any>(
  callback: T,
  deps: React.DependencyList,
  options: { name?: string; verbose?: boolean; devOnly?: boolean } = {},
): T {
  ...
  return useMemo(() => {
    ...
    if (verbose || recreationCount.current > 1) {
      console.log(
        `[useTrackedCallback] "${name}" recreation #${recreationCount.current}`,
      );
      ...
      if (recreationCount.current > 1) {
        console.groupCollapsed(
          `Callback "${
            name
          }" was recreated. Check dependency array to ensure it only changes when necessary.`,
        );
    ...
    return callback;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [callback, enabled, name, verbose, ...deps]);
}
