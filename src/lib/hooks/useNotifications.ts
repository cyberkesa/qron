...
export function useNotifications() {
       ...
       timeout: number = 3000) => {
        ...
        setNotifications((prev) => {
          ...
          if (updatedNotifications.length > 3) {
            return updatedNotifications.slice(
                -3);  // Оставляем только последние 3
          }
  ...
  const showSuccess = useCallback((message: string, timeout?: number) => {
  ...
  const showInfo = useCallback((message: string, timeout?: number) => {
    return showNotification(message, 'info', timeout);
  }, [showNotification]);

  const showWarning = useCallback((message: string, timeout?: number) => {
