...
export function useCart(): UseCartResult {
  ...
  const clearCart = async (): Promise<void> => {
    try {
      if (isAuthenticated) {
        ...
        try {
          // Используем Promise.allSettled вместо Promise.all для обработки
          // частичных ошибок
          const results = await Promise.allSettled(
            edges.map((edge: CartItemEdge) =>
              removeFromCartMutation({
                variables: { productId: edge.node.product.id },
                // Отключаем уведомления для каждого отдельного удаления
                onError: (error) => {
                  console.error(
          ...
          if (failedCount > 0) {
            console.warn(
              `${failedCount} из ${edges.length} товаров не удалось удалить`,
            );

            if (failedCount === edges.length) {
    ...
    const syncGuestCartToServer = async () => {
      ...
      if (isAuthenticated && guestCart.length > 0 && !cartLoading) {
        try {
          ...
          // Обновляем корзину с сервера (теперь полагаемся на update мутации)
          // await client.refetchQueries({
          //   include: ['GetCart'],
          // });
        } catch (error) {
  ...
  }, [isAuthenticated, cartLoading, guestCart, addToCartMutation, client]);
