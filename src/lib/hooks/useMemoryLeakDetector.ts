...
export function useMemoryLeakDetector(componentName: string) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (isDevMode && asyncOperations.current.size > 0) {
        console.warn(
            `[MemoryLeakDetector] ${componentName} was unmounted with ${
                asyncOperations.current
                    .size} pending async operations that might cause memory leaks:`,
            Array.from(asyncOperations.current));
      }
    };
  }, [componentName]);
  ...
  const trackAsyncOperation = <T extends(...args: any[]) => Promise<any>>(
      ...
      asyncFn:
          T): ((...args: Parameters<T>) => Promise<Awaited<ReturnType<T>>>) => {
    return async(...args: Parameters<T>): Promise<Awaited<ReturnType<T>>> => {
      if (!isMounted.current) {
        console.warn(`[MemoryLeakDetector] Starting async operation "${
            operationName}" on unmounted component ${componentName}`);
      }
      ...
      try {
        ...
        if (!isMounted.current) {
          console.warn(`[MemoryLeakDetector] Async operation "${
              operationName}" completed after component ${
              componentName} unmounted. Check for memory leaks.`);
        }
        ...
        if (!isMounted.current) {
          console.warn(
              `[MemoryLeakDetector] Async operation "${
                  operationName}" failed after component ${
