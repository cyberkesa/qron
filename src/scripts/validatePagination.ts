...
import {parse, visit, OperationDefinitionNode, ArgumentNode, ASTNode, ValueNode, SelectionSetNode,} from 'graphql';
...
function validateSelectionSet(
  ...
  const selectedFields =
      new Set(selectionSet.selections.filter(s => s.kind === 'Field')
...
function validatePaginationArgs(
    args: readonly ArgumentNode[]|undefined, fieldName: string): string[] {
  const errors: string[] = [];
  ...
  if (firstArg) {
    ...
    if (value !== null && value > MAX_PAGE_SIZE) {
      errors.push(`Field "${fieldName}" 'first' value (${
          value}) exceeds maximum allowed size (${MAX_PAGE_SIZE})`);
    }
  }
  ...
  if (lastArg) {
    ...
    if (value !== null && value > MAX_PAGE_SIZE) {
      errors.push(`Field "${fieldName}" 'last' value (${
          value}) exceeds maximum allowed size (${MAX_PAGE_SIZE})`);
    }
  }
        ...
        if (fieldType && isConnectionType(fieldType.name)) {
          ...
          if (edgesField && 'selectionSet' in edgesField) {
            const edgeErrors = validateSelectionSet(
                edgesField.selectionSet, REQUIRED_EDGE_SELECTIONS);
          ...
          if (pageInfoField && 'selectionSet' in pageInfoField) {
            const pageInfoErrors = validateSelectionSet(
                pageInfoField.selectionSet, REQUIRED_PAGE_INFO_SELECTIONS);
            if (pageInfoErrors.length > 0) {
              errors.push(`Field "${node.name.value}.pageInfo": ${
                  pageInfoErrors.join(', ')}`);
            }
          }
        }
...
console.log(`Общий результат: ${
    hasErrors ? '❌ Обнаружены ошибки в пагинации' :
