...
const PAGINATION_PATTERNS = [
  {
    ...
    extractValue: (match: RegExpMatchArray) => parseInt(match[1], 10),
    paramName: 'first'
  },
  {
...
function getComponentFiles(): string[] {
  ...
  return [...appFiles, ...componentFiles].map(
    ...
    for (const {pattern, extractValue, paramName} of PAGINATION_PATTERNS) {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (value > MAX_PAGE_SIZE) {
          ...
          errors.push(
              `File "${fileName}" line ${lineNumber}: ${paramName} value (${
...
function validateComponentsPagination() {
  ...
  console.log(`Общий результат: ${
      hasErrors ? '❌ Обнаружены ошибки в использовании пагинации' :
