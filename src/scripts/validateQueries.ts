...
import {DocumentNode, FragmentDefinitionNode, Kind, OperationDefinitionNode, parse} from 'graphql';
...
function validateField(
    typeName: string, fieldName: string, path: string[],
  ...
  if (type.fields) {
    ...
    if (!field) {
      context.errors.push(`Поле ${fieldName} не найдено в типе ${
          typeName} (путь: ${path.join('.')})`);
      return false;
    }

    // Проверяем deprecated поля
    if (field.isDeprecated) {
      context.warnings.push(`Поле ${fieldName} помечено как устаревшее (путь: ${
          path.join(
...
function validateFragment(
    fragment: FragmentDefinitionNode, context: ValidationContext): boolean {
  const typeName = fragment.typeCondition.name.value;
  return validateSelectionSet(
      fragment.selectionSet, typeName, [fragment.name.value], context);
}

// Функция для рекурсивной проверки полей
function validateSelectionSet(
    selectionSet: any, typeName: string, path: string[],
    context: ValidationContext): boolean {
  ...
  for (const selection of selectionSet.selections) {
    if (selection.kind === Kind.FIELD) {
      ...
      if (selection.arguments && selection.arguments.length > 0) {
        ...
        if (field && field.args) {
          for (const arg of selection.arguments) {
            const argDef =
                field.args.find((a: any) => a.name === arg.name.value);
            if (!argDef) {
              context.errors.push(
                  `Неизвестный аргумент ${arg.name.value} для поля ${
                      fieldName} (путь: ${currentPath.join('.')})`);
              isValid = false;
            } else if (
                arg.value.kind === Kind.ENUM && argDef.type.kind === 'ENUM') {
              ...
              if (!validateEnumValue(enumType, arg.value.value)) {
                context.errors.push(
                    `Неверное значение enum ${arg.value.value} для аргумента ${
                        arg.name.value} (путь: ${currentPath.join('.')})`);
                isValid = false;
              }
            }
          }
      ...
      if (selection.selectionSet) {
        ...
        isValid =
            validateSelectionSet(
                selection.selectionSet, fieldType.name, currentPath, context) &&
            isValid;
      }
    } else if (selection.kind === Kind.FRAGMENT_SPREAD) {
      const fragment = context.fragments.get(selection.name.value);
      if (!fragment) {
        context.errors.push(`Фрагмент ${
            selection.name.value} не найден (путь: ${path.join('.')})`);
        isValid = false;
      } else {
      ...
      isValid = validateSelectionSet(
                    selection.selectionSet, fragmentTypeName, path, context) &&
          isValid;
    }
  }
...
function validateVariableType(variableType: any, schemaType: any): boolean {
  ...
  if (variableType.kind === 'NamedType' &&
      (schemaType.kind === 'SCALAR' || schemaType.kind === 'INPUT_OBJECT' ||
...
function validateQuery(queryString: string):
  ...
  try {
    ...
    const context: ValidationContext =
        {seenFields: new Set(), fragments: new Map(), errors: [], warnings: []};
...
async function validateQueries() {
  ...
  for (const file of files) {
    ...
    for (const match of matches) {
      ...
      if (result.isValid) {
        validQueries++;
        console.log(`${colors.green('✓')} ${colors.gray(file)}: ${
            colors.green('Валидный запрос')}`);
      } else {
        hasErrors = true;
        console.log(`${colors.red('✗')} ${colors.gray(file)}: ${
            colors.red('Ошибки в запросе')}`);
